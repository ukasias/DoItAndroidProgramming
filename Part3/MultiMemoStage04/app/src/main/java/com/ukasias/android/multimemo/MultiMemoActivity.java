...
public class MultiMemoActivity extends AppCompatActivity {
    ...
    private int loadMemoListData() {
        ...
        if (mDatabase != null) {
            ...
            for (int i = 0; i < recordCount; i++) {
                ...
                if (dateTime != null) {
                    print("loadMemoListData() - dateTime: " + dateTime);
                ...
                adapter.addItem(new MemoListItem(id, dateTime, text,
                        id_photo, uri_photo,
                        id_video, uri_video,
                        id_voice, uri_voice,
    ...
    private String getPhotoUriStr(String id_photo) {
        ...
        if ((id_photo != null) && !id_photo.equals("-1")) {
            ...
            if (resCursor != null && resCursor.moveToNext()) {
    ...
    private String getHandWritingUriStr(String id_handwriting) {
        String handwritingUriStr = "";

        if ((id_handwriting != null) && !id_handwriting.equals("-1")) {
            String SQL =
                    "select URI from " + MemoDatabase.TABLE_HANDWRITING +
                            " where _id = " + id_handwriting + "";
            Cursor resCursor = mDatabase.rawQuery(SQL);
            if (resCursor != null && resCursor.moveToNext()) {
                handwritingUriStr = resCursor.getString(0);
                resCursor.close();
            }
        }
    ...
    private String getVideoUriStr(String id_video) {
        ...
        if ((id_video != null) && !id_video.equals("-1")) {
            String SQL =
                    "select URI from " + MemoDatabase.TABLE_VIDEO +
                            " where _id = " + id_video + "";
            Cursor resCursor = mDatabase.rawQuery(SQL);
            if (resCursor != null && resCursor.moveToNext()) {
                videoUriStr = resCursor.getString(0);
                resCursor.close();
            }
        }
        return videoUriStr;
    ...
    private String getVoiceUriStr(String id_voice) {
        ...
        if ((id_voice != null) && !id_voice.equals("-1")) {
            String SQL =
                    "select URI from " + MemoDatabase.TABLE_VOICE +
                            " where _id = " + id_voice + "";
            Cursor resCursor = mDatabase.rawQuery(SQL);
            if (resCursor != null && resCursor.moveToNext()) {
                voiceUriStr = resCursor.getString(0);
                resCursor.close();
            }
        }
        return voiceUriStr;
    ...
    private void viewMemo(int position) {
        ...
        print("video id: " + item.getData(4));
        print("video uri: " + item.getData(5));
        print("voice id: " + item.getData(2));
        print("voice uri: " + item.getData(3));
        print("handwriting id: " + item.getData(8));
        print("handwriting uri: " + item.getData(9));
